...
def localizacion(a,b,c):
   ...
   las antenas
    ...
    messagebox.showinfo(message="Se ha detectado un libro fuera de su area correspondiente", title="Alerta-Entorno")

    libros = Libro.objects.all().values()
    libros = [servicio['titulo'] for servicio in libros]

    print("LIBROS2_--------------->>>>>>>>>>>>>>>>>", libros)
    ...
    messagebox.showinfo(message="Se ha detectado el libro: (%s)" %random.choice(libros), title="Entorno-Libro fuera de área")
